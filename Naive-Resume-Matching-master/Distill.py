...
def remove_stopwords(text, stopwords=stop_words, optional_params=False, optional_words=['Email','email','Yahoo','yahoo','gmail','Gmail','outlook','Outlook']):
    if optional_params:
...
def remove_tags(text, postags=['PROPN', 'NOUN', 'ADJ', 'VERB', 'ADV']):
    ...
    """
    filtered = []
    if detect(" ".join(text))=="fr":
        str_text = nlp2(" ".join(text))
    else:
        str_text = nlp(" ".join(text))
